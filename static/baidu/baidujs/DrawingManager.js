...
(function() {
    ...
    switch($("input[name='drawType']:checked").val())
    {
    case "0":
        var styleOptions1 = {
            strokeColor:$("#pipeColor").val()||"red",    //边线颜色。
            fillColor:"",      //填充颜色。当参数为空时，圆形将没有填充效果。
            strokeWeight: $("#pipeWidth").val()||3,       //边线的宽度，以像素为单位。
            strokeOpacity: 0.8,    //边线透明度，取值范围0 - 1。
            fillOpacity: 0.6,      //填充的透明度，取值范围0 - 1。
            strokeStyle: 'solid' //边线的样式，solid或dashed。
        }
        styleOptions1.strokeStyle=($("#pipeType").val()=="fullLine")?'solid':"dashed"
        var me           = this,
            map          = this._map,
            mask         = this._mask,
            points       = [],   //用户绘制的点
        ...
        var startAction = function (e) {
            ...
            if (points.length == 1) {
                ...
                map.addOverlay(marker); 
                var label = new BMap.Label($("#pipeName").val(),{offset:new BMap.Size(20,-10)});
        ...
        var dblclickAction = function (e) {
            ...
            //draw[a]=drawPoint.slice(0,drawPoint.length-2)
            //console.log(draw)
            //console.log(drawPoint.slice(0,drawPoint.length-2));
            //console.log(drawPoint.slice(0,drawPoint.length-2));

            $.ajax({
            url:'/api/pipes',
            data:
                {name:$("#pipeName").val(),tenantId:tenantId,pipeColor:styleOptions1.strokeColor,pipeWidth:styleOptions1.strokeWeight,pipeType:styleOptions1.strokeStyle,drawPoint:JSON.stringify(draw)},
            type:'POST',//提交方式
            dataType: 'json',
    ...
    case "1":
        var styleOptions1 = {
            strokeColor:$("#patroltrackColor").val()||"red",    //边线颜色。
            fillColor:"",      //填充颜色。当参数为空时，圆形将没有填充效果。
            strokeWeight: $("#patroltrackWidth").val()||3,       //边线的宽度，以像素为单位。
            strokeOpacity: 0.8,    //边线透明度，取值范围0 - 1。
            fillOpacity: 0.6,      //填充的透明度，取值范围0 - 1。
            strokeStyle: 'solid' //边线的样式，solid或dashed。
        }
        styleOptions1.strokeStyle=($("#patroltrackType").val()=="fullLine")?'solid':"dashed"
        var me           = this,
            map          = this._map,
            mask         = this._mask,
            points       = [],   //用户绘制的点
        ...
        var startAction = function (e) {
            ...
            if (points.length == 1) {
                ...
                var marker = new BMap.Marker(e.point,{icon:myIcon}); // 创建点
                map.addOverlay(marker); 
                var label = new BMap.Label($("#patroltrackName").val(),{offset:new BMap.Size(20,-10)});
                marker.setLabel(label);
                if (me._drawingType == BMAP_DRAWING_POLYLINE) {
        ...
        var dblclickAction = function (e) {
            ...
            $.ajax({
            url:'/api/patroltracks',
            data:
                {name:$("#patroltrackName").val(),tenantId:tenantId,pipeColor:styleOptions1.strokeColor,pipeWidth:styleOptions1.strokeWeight,pipeType:styleOptions1.strokeStyle,drawPoint:JSON.stringify(draw)},
            type:'POST',//提交方式
            dataType: 'json',
            success: function(req){
               console.log(req)
